...
def kmeans_cluster(data, converge_dist, m, k):
    ...
    #for i in range (0,int(k)):
        #cp.append(data.takeSample(False, 1))
    cp = data.takeSample(False, k)
    #np.savetxt('firstCenter', cp);
    temp_dist = 1.0
...
if __name__ == "__main__":
    ...
    cp, output = kmeans_cluster(data, 0.01, m, int(sys.argv[4]))
    output.saveAsTextFile(sys.argv[2])
    np.savetxt('cp.txt', cp, fmt="%.18f %.18f")
    sc.stop()
